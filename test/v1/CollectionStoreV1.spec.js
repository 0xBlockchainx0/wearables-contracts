...
const Store = artifacts.require('DummyCollectionStoreV1')

describe.only('CollectionStoreV1', function () {
  // Contract
  let manaContract
  let storeContract
  let collection1
  ...
  beforeEach(async function () {
    ...
    collection1Beneficiary = accounts[ADDRESS_INDEXES.bidder]
    collection2Beneficiary = accounts[ADDRESS_INDEXES.anotherBidder]
  ...
  describe('Deploy', async function () {
    it('deploy with correct values', async function () {
      ...
      for (let i = 0; i < WEARABLES.length; i++) {
        ...
        expect(collectionData.beneficiary).to.be.equal(collection1Beneficiary)
      ...
      for (let i = 0; i < COLLECTION2_WEARABLES.length; i++) {
        ...
        expect(collectionData.beneficiary).to.be.equal(collection2Beneficiary)
        expect(collectionData.availableQty).to.be.eq.BN(
  ...
  describe('buy', function () {
    it('should buy', async function () {
      ...
      expect(logs[0].args._to).to.be.equal(collection1Beneficiary)
      ...
      await collection1BeneficiaryBalance.requireIncrease(finalPrice)
  ...
  describe('setCollectionData', function () {
    it('should set collection data', async function () {
      ...
      expect(item1.beneficiary).to.be.eq.BN(ZERO_ADDRESS)
      ...
      expect(item3.beneficiary).to.be.eq.BN(ZERO_ADDRESS)
      ...
      expect(item2.beneficiary).to.be.eq.BN(ZERO_ADDRESS)
      ...
      expect(logs[0].args._collectionBeneficiary).to.be.equal(anotherBuyer)
      ...
      expect(item1.beneficiary).to.be.eq.BN(anotherBuyer)
      ...
      expect(item3.beneficiary).to.be.eq.BN(anotherBuyer)
      ...
      expect(item2.beneficiary).to.be.eq.BN(anotherBuyer)
    ...
    it('should update collection data', async function () {
      ...
      expect(item1.beneficiary).to.be.eq.BN(collection1Beneficiary)
